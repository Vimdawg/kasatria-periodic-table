...
function generateTriangularSurface(vertex1: Vector3, vertex2: Vector3, vertex3: Vector3, numPoints: number): Vector3[] {
  ...
  // Calculate optimal grid size for the triangle
...
export function getTetrahedronLayout(count: number): LayoutTarget[] {
  ...
  // Tetrahedron parameters - perfect regular tetrahedron
  ...
  // Generate points for each face with complete surface coverage
  let pointIndex = 0
  
  // Face 1: Base face (bottom triangle)
  const baseFacePoints = generateTriangularSurface(baseVertices[0], baseVertices[1], baseVertices[2], pointsPerFace + (remainingPoints > 0 ? 1 : 0))
  ...
  const side1Points = generateTriangularSurface(baseVertices[0], baseVertices[1], apex, pointsPerFace + (remainingPoints > 1 ? 1 : 0))
  ...
  const side2Points = generateTriangularSurface(baseVertices[1], baseVertices[2], apex, pointsPerFace + (remainingPoints > 2 ? 1 : 0))
  ...
  const side3Points = generateTriangularSurface(baseVertices[2], baseVertices[0], apex, pointsPerFace + (remainingPoints > 3 ? 1 : 0))
